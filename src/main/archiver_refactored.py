#!/usr/bin/env python3
"""
Pi-Archiver Ultra - Рефакторенная версия
Основной модуль с разделенной архитектурой для удобства разработки
"""
...
# Импорты новых модульных компонентов
from archive_operations.archive_core import ArchiveOperations
...
class PiArchiverUltra:
    """Основной класс архиватора Pi-Archiver Ultra (рефакторенная версия)"""
    ...
    def __init__(self, cache_dir: str = None, 
        ...
        # Инициализируем операционные модули
        self.archive_ops = ArchiveOperations(self.pi_generator, self.compression_core, self.search_engine)
    ...
    def archive_file(self, input_path: str, output_name: Optional[str] = None, 
        ...
        """
        Архивирует один файл с использованием модульной архитектуры
        ...
        # 1. Генерируем π с использованием ArchiveOperations
        pi_digits = self.archive_ops.generate_pi_for_compression(
        ...
        # 3. Сжимаем данные с использованием ArchiveOperations
        blocks, stats, real_xor_key = self.archive_ops.compress_file_data(
    ...
    def extract_file(self, archive_name: str, output_dir: str = ".") -> List[str]:
        """
        Разархивирует архив с использованием модульной архитектуры
    ...
    def print_system_info(self, num_workers: int = None):
        """Выводит информацию о системе с использованием ArchiveOperations"""
        self.archive_ops.print_system_info(num_workers)
    
    def get_optimal_workers(self) -> int:
        """Возвращает оптимальное количество потоков"""
        return self.archive_ops.get_optimal_workers()
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Pi-Archiver Ultra - Архиватор на основе числа π (рефакторенная версия)')
    ...
    try:
        if args.command == 'archive':
            if len(args.files) == 1:
                ...
                # TODO: Реализовать archive_multiple_files в новой архитектуре
                print("Мультиархивация в рефакторенной версии пока не реализована")
