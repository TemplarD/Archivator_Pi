#!/usr/bin/env python3
"""
Pi-Archiver Ultra - Улучшенная версия с правильной многопоточностью
Использует микро-модульную архитектуру и оптимизацию для многопроцессорных систем
"""
...
from archive_operations.pi_generation.pi_generator_ops import PiGeneratorOperations
from archive_operations.data_compression.compression_ops import DataCompressionOperations
from archive_operations.system_info.system_info_ops import SystemInfoOperations
from archive_operations.system_info.multi_core_info import MultiCoreInfo
from archive_operations.pi_generation.enhanced_pi_generator import EnhancedPiGenerator
from extract_operations.extract_core import ExtractOperations
...
class PiArchiverUltraEnhanced:
    """Основной класс архиватора Pi-Archiver Ultra (улучшенная версия)"""
    ...
    def __init__(self, cache_dir: str = None, 
        ...
        self.enhanced_pi_ops = EnhancedPiGenerator(self.pi_generator)
    ...
    def archive_file(self, input_path: str, output_name: Optional[str] = None, 
                    ...
                    force_regenerate: bool = False, auto_optimize: bool = True) -> str:
        """
        Архивирует один файл с использованием улучшенной многопоточности
        ...
        # 1. Генерируем π с использованием EnhancedPiGenerator
        pi_digits = self.enhanced_pi_ops.generate_pi_enhanced(
            self.pi_precision, use_gpu, force_regenerate=force_regenerate, 
    ...
    def extract_file(self, archive_name: str, output_dir: str = ".") -> List[str]:
        """
        Разархивирует архив с использованием улучшенной архитектуры
    ...
    def print_system_info(self, num_workers: int = None):
        """Выводит подробную системную информацию"""
        self.multi_core_info.print_detailed_system_info(num_workers)
    
    def get_optimal_workers(self, task_type: str = "general") -> int:
        """Возвращает оптимальное количество потоков для задачи"""
        return self.multi_core_info.get_optimal_workers(task_type)
    
    def benchmark_threading(self, pi_precision: int = 100000) -> Dict:
        """Тестирует производительность с разным количеством потоков"""
        return self.enhanced_pi_ops.benchmark_threading(pi_precision)
...
def main():
    ...
    archive_parser.add_argument("--no-optimize", action="store_true", help="Отключить автоматическую оптимизацию")
    ...
    # Команда бенчмарка
    benchmark_parser = subparsers.add_parser('benchmark', help='Бенчмарк многопоточности')
    benchmark_parser.add_argument("--precision", type=int, default=100000, help="Количество цифр для теста")
    ...
    # Создаем архиватор
    archiver = PiArchiverUltraEnhanced(pi_precision=getattr(args, 'precision', 1000000))
        ...
        if args.command == 'archive':
            if len(args.files) == 1:
                archiver.archive_file(
                    ...
                    not getattr(args, "no_optimize", False)
                )
            else:
                output_name = args.output or f"archive_{int(time.time())}.piarc"
                print("Мультиархивация в улучшенной версии пока не реализована")
            ...
            archiver.benchmark_threading(args.precision)
